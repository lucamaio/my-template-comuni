...
<?php foreach (($messages ?? []) as $message): ?>
    ...
    if ($message_date && $message_date <= $now) {
        continue;
    }

    $colore = $message['colore_message'] ?? '';
    $color = ($colore === 'yellow') ? 'black' : 'white';

    $icona = !empty($message['icona_message']);
    $link = $message['link_message'] ?? '';
    ?>

    <div class="p-3 home-message <?php echo esc_attr($colore); ?>">
        <div class="home-message-content">
            <p class="msg">
                ...
                <?php echo nl2br(esc_html($testo)); ?>

                <?php if (!empty($link)): ?>
                    <a href="<?php echo esc_url($link); ?>" class="btn btn-sm btn-outline-<?php echo esc_attr($color); ?> ms-3">
                        Dettagli
                    </a>
                <?php endif; ?>
