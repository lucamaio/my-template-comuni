...
add_action('before_delete_post', 'dci_cleanup_persona_pubblica_relations');
function dci_cleanup_persona_pubblica_relations($post_id)
{
    if (get_post_type($post_id) !== 'persona_pubblica') {
        return;
    }
    ...
    $relation_map = array(
        'unita_organizzativa' => array(
            '_dci_unita_organizzativa_responsabile' => 'multi',
            '_dci_unita_organizzativa_assessore_riferimento' => 'single',
            '_dci_unita_organizzativa_persone_struttura' => 'multi',
        ),
        'incarico' => array(
            '_dci_incarico_persona' => 'single',
        ),
        'punto_contatto' => array(
            '_dci_punto_contatto_persona' => 'single',
        ),
        'documento_pubblico' => array(
            '_dci_documento_pubblico_autori' => 'multi',
        ),
        'notizia' => array(
            '_dci_notizia_persone' => 'multi',
        ),
        'evento' => array(
            '_dci_evento_persone' => 'multi',
        ),
        'consiglio' => array(
            '_dci_consiglio_partecipanti' => 'multi',
        ),
    );

    foreach ($relation_map as $post_type => $meta_keys) {
        ...
        foreach ($linked_posts as $linked_post_id) {
            foreach ($meta_keys as $meta_key => $relation_type) {
                $current_value = get_post_meta($linked_post_id, $meta_key, true);

                if ($relation_type === 'multi') {
                    if (!is_array($current_value)) {
                        $current_value = empty($current_value) ? array() : array($current_value);
                    }

                    $normalized = array_values(array_filter(array_map('intval', $current_value)));

                    if (!in_array((int) $post_id, $normalized, true)) {
                        continue;
                    }

                    $normalized = array_values(array_diff($normalized, array((int) $post_id)));

                    if (empty($normalized)) {
                        delete_post_meta($linked_post_id, $meta_key);
                    } else {
                        update_post_meta($linked_post_id, $meta_key, $normalized);
                    }

                    continue;
                }

                if ((int) $current_value === (int) $post_id) {
                    delete_post_meta($linked_post_id, $meta_key);
                }
            }
        }
